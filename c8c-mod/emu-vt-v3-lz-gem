...
// Enum to identify which flag operation is pending
typedef enum {
    LAZY_FLAG_NONE,
    LAZY_FLAG_ADD,
    LAZY_FLAG_SUB,
...
// State for the pending flag calculation
static LazyFlagOp pending_flag_op = LAZY_FLAG_NONE;
static uint8_t flag_op_x_val; // Stores the pre-operation value of Vx
static uint8_t flag_op_y_val; // Stores the pre-operation value of Vy
...
// This function calculates and sets the V[F] flag based on a pending operation
static void evaluate_lazy_flag()
{
    if (pending_flag_op == LAZY_FLAG_NONE) {
        return; // Nothing to do
    }

    switch (pending_flag_op) {
        case LAZY_FLAG_ADD:
            // Set VF to 1 if there was a carry, 0 otherwise.
            v[0xF] = ((uint16_t)flag_op_x_val + flag_op_y_val > 0xFF);
            break;
        case LAZY_FLAG_SUB:
            // Set VF to 1 if there was NO borrow (Vx >= Vy), 0 otherwise.
            v[0xF] = (flag_op_x_val >= flag_op_y_val);
            break;
        case LAZY_FLAG_SUBN:
            // Set VF to 1 if there was NO borrow (Vy >= Vx), 0 otherwise.
            v[0xF] = (flag_op_y_val >= flag_op_x_val);
            break;
        case LAZY_FLAG_SHR:
            // Set VF to the least significant bit of Vy before the shift.
            v[0xF] = flag_op_y_val & 0x01;
            break;
        case LAZY_FLAG_SHL:
            // Set VF to the most significant bit of Vy before the shift.
            v[0xF] = (flag_op_y_val >> 7) & 0x01;
            break;
        case LAZY_FLAG_NONE: // Should not be reached
            break;
    }

    // The flag has been calculated, so reset the state.
    pending_flag_op = LAZY_FLAG_NONE;
}
...
// MODIFIED for lazy flag evaluation
static void _8XY4() { uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4; pending_flag_op = LAZY_FLAG_ADD; flag_op_x_val = v[x]; flag_op_y_val = v[y]; v[x] += v[y]; }
// MODIFIED for lazy flag evaluation
static void _8XY5() { uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4; pending_flag_op = LAZY_FLAG_SUB; flag_op_x_val = v[x]; flag_op_y_val = v[y]; v[x] -= v[y]; }
// MODIFIED for lazy flag evaluation
static void _8XY7() { uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4; pending_flag_op = LAZY_FLAG_SUBN; flag_op_x_val = v[x]; flag_op_y_val = v[y]; v[x] = v[y] - v[x]; }
// MODIFIED for lazy flag evaluation
static void _8XY6() { uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4; pending_flag_op = LAZY_FLAG_SHR; flag_op_y_val = v[y]; v[x] = v[y] >> 1; }
// MODIFIED for lazy flag evaluation
static void _8XYE() { uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4; pending_flag_op = LAZY_FLAG_SHL; flag_op_y_val = v[y]; v[x] = v[y] << 1; }
...
static void cycle()
{
    // Evaluate any pending flag calculation from the *previous* cycle.
    // This makes the flag available for the current instruction and
    // correctly handles quirks like `SHR Vx, Vx`.
    evaluate_lazy_flag();
...
/*
An excellent idea! Lazy flag evaluation is a great way to handle some of the original CHIP-8's quirks, especially regarding the shift instructions where Vx and Vy might be the same register.

Here is the code modified to include lazy flag evaluation. The main changes are:

    An enum and state variables are added to track pending flag calculations.
    A new function, evaluate_lazy_flag(), is introduced to perform the deferred calculation.
    The main cycle() function now calls evaluate_lazy_flag() at the start of each cycle to ensure the flag from the previous instruction is correctly set before the current instruction executes.
    The arithmetic and shift opcodes (8XY4, 8XY5, 8XY6, 8XY7, 8XYE) have been updated to register a pending flag calculation instead of setting V[0xF] immediately.
    A subtle bug in the original borrow logic for subtraction opcodes has also been corrected as part of this change.
